# Product Requirements Document: bootsralph v2 MVP

## Executive Summary

**Product**: bootsralph v2
**Version**: 2.0.0
**Target Release**: Q1 2026
**Status**: Planning

bootsralph v2 is a complete rewrite that transforms from a monolithic CLI orchestrator into a thin glue layer that delegates planning and execution to Ralph TUI while providing intelligent stack scaffolding, configuration generation, and bidirectional skills synchronization.

## Problem Statement

bootsralph v1 attempted to build its own TUI, PRD management, and execution engine. This led to:
- Feature duplication with Ralph TUI
- Maintenance burden for TUI components
- Complex codebase with mixed responsibilities
- Limited extensibility

**Core insight**: Ralph TUI already does planning and execution excellently. bootsralph should focus on what it does best: stack detection, scaffolding, and skills management.
...
So that I understand what bootsralph is doing at all times
...
I want bootsralph to detect my stack automatically
...
Acceptance Criteria:
- Manifest stored in .bootsralph/skills-manifest.json
...
```
As a Ralph TUI user
I want a bootsralph-stack skill with my project's stack context
So that Ralph TUI can make informed decisions

Acceptance Criteria:
- Generates .ralph-tui/skills/bootsralph-stack/SKILL.md
...
Acceptance Criteria:
- Adds postinstall script: "bootsralph sync --quiet"
- Adds prepare script: "lefthook install && bootsralph sync --quiet"
...
**US-10: Bootsralph Config Generation**
```
As a developer
...
Acceptance Criteria:
- Generates .bootsralph/config.json
- Includes: version, createdAt, full StackConfig object
- Pretty-printed JSON for human readability

Tasks:
- P4-06: Implement src/generators/bootsralph-config.ts (write config.json)
```
...
Acceptance Criteria:
- Command: bootsralph create <name>
...
- After scaffolding:
  ...
  - Generate lefthook.yml, ralph skill, bootsralph config
...
5. **Ralph TUI integration works**: Can run `ralph-tui create-prd --prd-skill bootsralph-stack`

### Key Metrics
- Time from `bootsralph create` to `ralph-tui run`: < 2 minutes
...
### .ralph-tui/skills/bootsralph-stack/SKILL.md
```markdown
# bootsralph Stack Context

This project was scaffolded with bootsralph.
