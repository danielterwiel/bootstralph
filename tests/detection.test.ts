...
async function createTempDir(): Promise<string> {
  const prefix = join(tmpdir(), 'bootsralph-test-');
...
describe('inferStack', () => {
  describe('Framework Detection', () => {
    ...
    it('should detect Next.js with higher confidence when config exists', () => {
      ...
      expect(stackWithConfig.confidence).toBeGreaterThan(stackWithoutConfig.confidence);
    });
  ...
  describe('Full Stack Detection', () => {
    ...
    it('should detect complete API stack', () => {
      ...
      const configFiles = ['drizzle.config.ts', 'vitest.config.ts', 'tsconfig.json'];
